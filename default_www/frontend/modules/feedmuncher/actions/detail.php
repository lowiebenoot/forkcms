...
class FrontendFeedmuncherDetail extends FrontendBaseBlock
{
	...
	private function getData()
	{
		...
		{
			// get data
			$this->record = FrontendFeedmuncherModel::getDraft($this->URL->getParameter(1), $this->URL->getParameter('draft', 'int'));
		...
		{
			// get data
			$this->record = FrontendFeedmuncherModel::getRevision($this->URL->getParameter(1), $this->URL->getParameter('revision', 'int'));
		...
		// get by URL
		else $this->record = FrontendFeedmuncherModel::get($this->URL->getParameter(1));
		...
		// get tags
		$this->record['tags'] = FrontendTagsModel::getForItem('feedmuncher', $this->record['revision_id']);

		// get comments
		$this->comments = FrontendFeedmuncherModel::getComments($this->record['id']);

		// get settings
		$this->settings = FrontendModel::getModuleSettings('feedmuncher');
		...
		if($this->frm->isSubmitted())
		{
			...
			if($this->frm->isCorrect())
			{
				...
				if($moderationEnabled)
				{
					...
					if(!FrontendFeedmuncherModel::isModerated($author, $email)) $comment['status'] = 'moderation';
				}
				...
				// insert comment
				$comment['id'] = FrontendFeedmuncherModel::insertComment($comment);
