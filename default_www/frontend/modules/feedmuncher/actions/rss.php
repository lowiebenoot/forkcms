 ...
 * @since		2.0
 */
class FrontendFeedmuncherRSS extends FrontendBaseBlock
{
	/**
	 * The articles
	 *
	...
	{
		// get articles
		$this->items = FrontendFeedmuncherModel::getAll(30);
	...
	{
		// get vars
		$title = (isset($this->settings['rss_title_' . FRONTEND_LANGUAGE])) ? $this->settings['rss_title_' . FRONTEND_LANGUAGE] : FrontendModel::getModuleSetting('feedmuncher', 'rss_title_' . FRONTEND_LANGUAGE, SITE_DEFAULT_TITLE);
		...
		foreach($this->items as $item)
		{
			...
			// meta is wanted
			if(FrontendModel::getModuleSetting('feedmuncher', 'rss_meta_' . FRONTEND_LANGUAGE, true))
			{
				// append meta
