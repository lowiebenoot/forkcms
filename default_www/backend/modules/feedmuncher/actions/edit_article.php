...
class BackendFeedmuncherEditArticle extends BackendBaseActionEdit
{
	...
	private function getData()
	{
		...
		if($revisionToLoad !== null)
		{
			...
			$this->record = (array) BackendFeedmuncherModel::getArticleRevision($this->id, $revisionToLoad);
		...
		{
			// overwrite the current record
			$this->record = (array) BackendFeedmuncherModel::getArticleRevision($this->id, $draftToLoad);
	...
	private function loadDrafts()
	{
		...
		// add use column
		$this->dgDrafts->addColumn('use_draft', null, ucfirst(BL::lbl('UseThisDraft')), BackendModel::createURLForAction('edit_article') . '&amp;id=[id]&amp;draft=[revision_id]', BL::lbl('UseThisDraft'));
	...
	{
		// create datagrid
		$this->dgRevisions = new BackendDataGridDB(BackendFeedmuncherModel::QRY_DATAGRID_BROWSE_REVISIONS, array('archived', $this->record['id'], BL::getWorkingLanguage(), 'feedmuncher'));
		...
		// set colum URLs
		$this->dgRevisions->setColumnURL('title', BackendModel::createURLForAction('edit_article') . '&amp;id=[id]&amp;revision=[revision_id]');
		...
		// add use column
		$this->dgRevisions->addColumn('use_revision', null, ucfirst(BL::lbl('UseThisVersion')), BackendModel::createURLForAction('edit_article') . '&amp;id=[id]&amp;revision=[revision_id]', BL::lbl('UseThisVersion'));
	}
	...
	private function validateForm()
	{
		...
		if($this->frm->isSubmitted())
		{
			...
			$this->meta->setUrlCallback('BackendFeedmuncherModel', 'getURL', array($this->record['id']));
			...
			if($this->frm->isCorrect())
			{
				...
				$item['date'] = BackendModel::getUTCDate(null, BackendModel::getUTCTimestamp($this->frm->getField('publish_on_date'), $this->frm->getField('publish_on_time')));
